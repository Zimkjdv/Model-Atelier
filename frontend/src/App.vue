...
<template>
  <div class="layout">
    <aside>
      ...
      <nav aria-label="主要導覽"><button v-for="(item, i) in pages" :key="item" :class="{ active: page === item }" @click="page = item"><span class="nav-icon">{{ ['◈', '▦', '▧', '◉', '⚙', '▨'][i] }}</span>{{ item }}<span v-if="i === 2" class="soon">待開發</span></button></nav>
    ...
    <main>
      ...
      <section class="content">
        ...
        <KeepAlive><CreationStudio v-if="page === '創作工作台'" @models="page = '模型庫'" @assets="page = '參考素材'" /></KeepAlive>
        ...
        <article v-else-if="page === '作品庫'" class="panel placeholder"><div class="outline-icon">◈</div><h2>{{ page }}尚未實作</h2><p>目前版本已提供系統資訊、模型庫與 ComfyUI 連線設定。<br>生成與作品保存將依開發清單接續實作。</p><button class="secondary" @click="page = '系統資訊'">查看系統資訊</button></article>
      </section>
    </main>
  </div>
</template>
